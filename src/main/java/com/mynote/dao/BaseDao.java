...
public class BaseDao {
     ...
     * 4.如果有参数，则设置参数，下标从1开始 （数组或集合、循环设置参数）
     * 5.执行查询，得到结果集
     * 6.得到结果集的元数据对象 (查询得到的字段数量以及查询了哪些字段)
     * 7.判断并分析结果集
     * 7.1实例化对象
     * 7.2遍历查询的字段数量，得到数据库中查询到的每一个列名
     * 7.3通过反射，使用列名得到对应的field对象
     * 7.4拼接set方法，得到字符串
     * 7.5通过反射，将set方法的字符串反射成类中的指定set方法
     * 7.6通过invoke调用set方法
     * 7.7将对应的JavaBean设置到集合中
     * 8.关闭资源
     *
     * @param sql    SQL语句
     * @param params 参数
    ...
    public static List queryRows(String sql, List<Object> params, Class cls) {
        ...
        try {
            ...
            //得到结果集的元数据对象 (查询得到的字段数量以及查询了哪些字段)
